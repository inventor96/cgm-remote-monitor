  ...
  function virtAsstForecastHandler (next, slots, sbx) {
    if (sbx.properties.loop.lastLoop.predicted) {
      ...
      if (endPrediction.valueOf() < sbx.time) {
        next('Loop Forecast', translate('virtAsstForecastUnavailable'));
      } else {
        ...
        var value = '';
        if (min === max) {
          value = translate('around') + ' ' + max;
        } else {
          value = translate('between') + ' ' + min + ' ' + translate('and') + ' ' + max;
        }
        var response = translate('virtAsstLoopForecast', {
          params: [
            value
